DROP POLICY IF EXISTS "Acesso por sigilo e escopo do perfil" ON public.ged_documents;

CREATE POLICY "Acesso por sigilo e escopo do perfil"
...
USING (
  ...
  AND (
    ...
    OR public.has_role_in_org(auth.uid(), 'org_admin'::app_role, organization_id)
        ...
        AND EXISTS (
          ...
          WHERE pe.perfil_id = perf.perfil_id
            AND (
              ...
              OR (pe.tipo_escopo = 'SETOR' AND EXISTS (
                    SELECT 1 FROM public.folders f
                    WHERE f.past_id = ged_documents.folder_id AND f.set_id = pe.referencia_id))
              OR (pe.tipo_escopo = 'DEPARTAMENTO' AND EXISTS (
                    SELECT 1 FROM public.folders f
