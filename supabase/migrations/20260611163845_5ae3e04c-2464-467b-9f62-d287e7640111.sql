DROP FUNCTION IF EXISTS public.dashboard_indicators(uuid);
...
BEGIN
  ...
  -- Páginas consumidas no período (calculado via auditoria ou documento_ocr se disponível)
  -- Para simplificar, vamos somar o numero_paginas dos docs criados no período
  SELECT COALESCE(SUM(numero_paginas), 0) INTO v_used_pages
    FROM public.ged_documents
    WHERE organization_id = p_org_id 
      AND status <> 'deleted'
      AND created_at::date BETWEEN p_start_date AND p_end_date;

  -- Documentos vencidos (geral, pois é status atual)
  ...
  FROM (
    SELECT 
      f.name,
      SUM(COALESCE(d.tamanho_arquivo, 0)) as bytes
    ...
    GROUP BY f.name
    HAVING SUM(COALESCE(d.tamanho_arquivo, 0)) > 0
    ORDER BY bytes DESC
    LIMIT 10
  ) t;
  ...
  FROM (
    SELECT 
      p.full_name as name,
      SUM(COALESCE(d.numero_paginas, 0)) as total
    ...
    GROUP BY p.full_name
    HAVING SUM(COALESCE(d.numero_paginas, 0)) > 0
    ORDER BY total DESC
    LIMIT 5
  ) t;
